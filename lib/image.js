...
Image.prototype.makeMpc = function (suffix) {
    var self = this;
    return this.gm.writeAsync(self.src + suffix + '.mpc')
        .then(function () {
            return gm(self.src + '.mpc');
        });
};
            ...
            if (typeof version.original !== 'undefined') {
                ...
                self.makeMpc(version.suffix)
                    .then(function(mpc){
                        ...
                        var img = mpc.resize(version.maxWidth, version.maxHeight, '^')
                            .gravity('Center')
                            .crop(version.maxWidth, version.maxHeight)
    ...
    return this.getMeta()
        ...
        .then(function(files){
            _.forEach(files, function(file){
                fs.unlink(file.src, function (err) {
                    if (err){
                        console.log('error when deleted : '+ file.src );
                    }
                    console.log('successfully deleted : '+ file.src );
                });
            });
            return files;
        })
