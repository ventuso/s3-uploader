...
Upload.prototype.getRandomName = function (length) {
    var input = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
    var res = res + input[Math.floor(Math.random() * input.length)];
...
Upload.prototype.uploadGeneratePath = function (prefix) {
    var path = prefix + this.opts.pathGenerator.call();
    if (path != '' && !_.endsWith(path, '/')) {
        path = path + '/';
    }
    var name = this.opts.nameGenerator.call();
    path = path + name;
    return this.uploadPathIsAvailable(prefix + this.getRandomPath())
